...
Xtr,Xte,ytr,yte = train_test_split(X.to_numpy(), y.to_numpy(), 
                                   train_size=1000, random_state=1,stratify=y)#データ分割
# %%
"""パラメータ設定※随時調整"""
output_folder='../../output/method2/drybeans4'
runlist=[1]
...
prob_para={'f1_score_average':'weighted',#f値の計測タイプ指定
           ...
           'subnum':100,'sub_low':0.01,'sub_high':0.05,
           'dt_depth':3, 'depth_low':4, 'penalty_dnum':500,
           'penalty_ac':0.7, 'penalty_sz':1000, 'penalty_fm':0,'penalty_fl':1,
...
print("トレーニングデータ：",problem.Xtr.shape)
print(np.unique(problem.ytr, return_counts=True))
print("テストデータ",problem.Xte.shape)
print(np.unique(problem.yte, return_counts=True))
print('目的数:',problem.nobj)
print('遺伝子長さ',problem.genelen)
"""GAアルゴリズム実行"""
...
"""グラフ作成フェーズ※随時設定"""
"""ほしい散布図リストを設定"""
xl=['AC(mk)','AC(ev)','AC(test)','F1(mk)','F1(ev)','F1(test)']#x軸
yl=['size',  'size',  'size',    'size',  'size',  'size']#y軸
"""ディレクトリ作成"""
os.mkdir('graph')
...
for g in genlist2:
    for x, y in zip(xl, yl):
        pltdf=df_all[df_all['run'].isin(runlist)&df_all['gen'].isin([g])]#dfをあるrunのあるgensに限定
        fig = plt.figure(figsize = (8,5))
